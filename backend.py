...
class CustomMultiVectorRetriever:
    ...
    def get_relevant_documents(self, query):
        ...
        for b in retrieved_bytes:
            if b is not None:
                parents.append(pickle.loads(b)) # Use pickle.loads to restore the object
...
class MultiRag:
    def __init__(self):
        ...
        self.vision_model=ChatOpenAI(model='gpt-4.1-mini')
    ...
    def refine(self,state:State)->State:
        ...
        return {
            ...
            'refined_context_docs': kept_docs # This goes to generate node
        }
    ...
    def generate(self,state: State) -> State:
        ...
        """
        question = state['question']
        relevant_docs = state.get('refined_context_docs', []) # Assuming this is your List[Document]
        history = state.get('messages', [])

    # 1. Sort context by modality
        image_context = []
        table_context = []
        text_context = []
...
def delete_thread(thread_id:str):
    """delete thread id from data base""" 
    ...
    try:
        ...
        for table in tables:
            try:
                # Use the string version 'target_id' here
                cursor.execute(f"DELETE FROM {table} WHERE thread_id = ?", (target_id,))
            except sqlite3.OperationalError:
                # Skip if table doesn't exist in your specific version
                continue
        ...
        print(f'error while detaling thread id{e}')
        return False
    ...
    # def response_fun(self,user_query):
        
    #     workflow=self.workflow_fun()
    #     response=workflow.invoke({"question": user_query,
    #     "docs": [],
    #     "good_docs": [],
    #     "verdict": "",
    #     "reason": "",
...
#     print('starting multi Rag')
#     rag=MultiRag()
    
#     # rag.build_enhanced_vector_database(image_paths=['Figure_1.png'],file_paths=['LIC_Claim_Process_Guide_CleanText.pdf'])
#     # print('completed data ingestion in vector')
    
#     # # question='List all animals shown in the image.'
#     # # response=rag.response_fun(question)
#     # # print("VERDICT:", response["verdict"])
#     # # print("REASON:", response["reason"])
#     # # print("WEB_QUERY:", response["web_query"])
#     # # print("\nOUTPUT:\n", response["answer"])

    
#     question='what is Batch normalization vs layer normalization'
